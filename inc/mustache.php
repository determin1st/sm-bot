...
class Mustache_Engine # {{{
{
     ...
     */
    public function render($template, $context = array(), $delims = null)
    {
      # change delimiters
      if ($delims)
      {
        $backup = $this->delimiters;
        $this->delimiters = $delims;
      }
      # operate
      $res = $this->loadTemplate($template)->render($context);
      # restore delims
      if ($delims) {
        $this->delimiters = $backup;
      }
      # done
      return $res;
    }
    ...
    private function loadSource($source, Mustache_Cache $cache = null)
    {
        ...
        if (!isset($this->templates[$className])) {
            ...
            $this->templates[$className] = new $class($this);
        }
