...
# determine fractal colors
#$colors = [imagecolorallocate($img, 0, 0, 0)];
$colors = [$bg];
for ($a = 1; $a < 33; $a++) {
  array_push($colors, imagecolorallocate($img, $a*8 - 1, 0, 0));
}
# prepare
$zoom  = 1.0;
$x_min = 1.0;
...
for ($x = 0; $x < $sx; $x++)
{
  for ($y = 0; $y < $sy; $y++)
  {
    $a = mandlebrot($x/$zx - 1, $y/$zy - 2.09);
    #$a = mandlebrot(($x - $px) / $zx, ($y - $py) / $zy);
    imagesetpixel($img, $x, $y, $colors[$a]);
  }
}
# complete
imagejpeg($img);
